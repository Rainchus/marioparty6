...
void GXSetTevOrder(GXTevStageID stage, GXTexCoordID coord, GXTexMapID map, GXChannelID color)
{
    ...
    if (stage & 1) {
        ...
        SET_REG_FIELD(1163, *ptref, 1, 18,
                      map != GX_TEXMAP_NULL && !(map & GX_TEX_DISABLE));
    } else {
...
{
    SET_REG_FIELD(1190, gx->genMode, 4, 10, nStages - 1);
    gx->dirtyState |= 4;
}
