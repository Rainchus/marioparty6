...
f32 PSVECMag(const register Vec *v)
{
	...
	asm {
		...
		lfs         square_mag, 8(v)
		fsubs       f2, half, half
		ps_madd     square_mag, square_mag, square_mag, f0
		ps_sum0     square_mag, square_mag, f0, f0
		fcmpu       cr0, square_mag, f2
		beq         _PSVECMag_end
		frsqrte     f0, square_mag
    }
#endif // clang-format on
    three = 3.0f;
#ifdef __MWERKS__ // clang-format off
    asm {
		fmuls       f2, f0, f0
		fmuls       f0, f0, half
		fnmsubs     f2, f2, square_mag, three
		fmuls       f0, f2, f0
		fmuls       square_mag, square_mag, f0
	_PSVECMag_end:
	}
#endif // clang-format on
    return square_mag;
