...
static void DEMOSetFontType(DMFontType attr)
{
    switch (attr) {
        case DM_FT_RVS:
...
static void DEMOLoadFont(enum _GXTexMapID texMap, enum _GXTexMtx texMtx, DMTexFlt texFlt)
{
    float fontTMtx[3][4];
    ...
    if (texFlt == 0) {
        GXInitTexObjLOD(&fontTexObj, 0, 0, 0.0f, 0.0f, 0.0f, 0, 0, 0);
        fontShift = 0;
    }
...
static void DEMOSetupScrnSpc(long width, long height, float depth)
{
    float pMtx[4][4];
    float mMtx[3][4];
    float offset;

    if (DEMOGetRenderModeObj()->field_rendering != 0 && VIGetNextField() == 0) {
        offset = -0.667f;
    } else {
        offset = 0.0f;
    }
    MTXOrtho(pMtx, offset, height, 0.0f, width, 0.0f, -depth);
