...
extern char lbl_1_data_0[];

enum {
    TITLE_HSF_PARTY = 0x00D50013,
    TITLE_HSF_STAR = 0x00D50014,
    TITLE_HSF_SIX = 0x00D50015,
    ...
    BOOT_DATA_DIR = 0x00D50000,
    BOARD_DATA_DIR = 0x00230000,
    NINTENDO_SPRITE_DATA = 0x00D50001,
    OPENING_WAIT_SPRITE_DATA = 0x00D50009,
    BOOT_SOUND_GROUP = 9,
};
...
char lbl_1_data_0[] = "******* Boot ObjectSetup *********\n";
char lbl_1_data_24[] = "%dms\n";
...
void BootExec(void)
{
    ...
    if (omovlevtno != 0) {
        HuAudSndGrpSetSet(BOOT_SOUND_GROUP);
        ...
        if (NintendoDispF == FALSE) {
            ...
            GameMesInit();
            HuWinInit(1);
            asyncStat = HuDataDirReadAsync(BOOT_DATA_DIR);
            while (OSTicksToMilliseconds(OSGetTick() - start) < 1500) {
                HuPrcVSleep();
            }
            OSReport(lbl_1_data_24, OSTicksToMilliseconds(OSGetTick() - start));
        } else {
...
title_exec:
    ...
    asyncStat = HuDataDirReadAsync(BOARD_DATA_DIR);
...
void BootProgExec(void)
{
    ...
    anim = HuSprAnimRead(HuDataSelHeapReadNum(0x00D50006, HU_MEMNUM_OVL, HEAP_MODEL));
    sprite = HuSprCreate(anim, 0, 0);
    HuSprGrpMemberSet(group, 0, sprite);
    HuSprPosSet(group, 0, 288.0f, 240.0f);

    anim = HuSprAnimRead(HuDataSelHeapReadNum(0x00D5000A, HU_MEMNUM_OVL, HEAP_MODEL));
    sprite = HuSprCreate(anim, 0, 0);
    HuSprGrpMemberSet(group, 1, sprite);
    HuSprPosSet(group, 1, posTbl[selection * 2], posTbl[selection * 2 + 1]);

    anim = HuSprAnimRead(HuDataSelHeapReadNum(0x00D5000B, HU_MEMNUM_OVL, HEAP_MODEL));
    sprite = HuSprCreate(anim, 0, 0);
    ...
    if (selection == 0) {
        anim = HuSprAnimRead(HuDataSelHeapReadNum(0x00D50007, HU_MEMNUM_OVL, HEAP_MODEL));
    } else {
        anim = HuSprAnimRead(HuDataSelHeapReadNum(0x00D50008, HU_MEMNUM_OVL, HEAP_MODEL));
    }
    sprite = HuSprCreate(anim, 0, 0);
    HuSprGrpMemberSet(group, 0, sprite);
    HuSprPosSet(group, 0, 288.0f, 240.0f);
