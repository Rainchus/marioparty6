...
typedef struct CharWork_s {
    ...
    s16 model; //0x02
    s16 motNoCurr; //0x04
    s16 motNoShiftCurr; //0x06
    s16 motNoPrev; //0x08
    s16 motNoShiftPrev; //0x0A
    HU3D_MOTIONID motId[CHAR_MOT_MAX]; //0xC
    s16 motNoTbl[CHAR_MOT_MAX]; //0x4C
    s16 vol; //0x8C
    s16 pan; //0x8E
    u8 voiceFlag[CHAR_MOT_MAX]; //0x90
    u32 attr; //0xB0
    s8 step; //0xB4
    HuVecF pos; //0xB8
    AMEM_PTR motAMemP; //0xC4
    HUPROCESS *process; //0xC8
    s16 timingHookNo; //0xCC
    s16 motNoPlay[2]; //0xCE
    s16 motPlayTime[2]; //0xD2
} CHAR_WORK;

typedef struct EffectData_s {
    unsigned int dataNum; //0x00
    s16 maxCnt; //0x04
    s16 blendMode; //0x06
    s32 motCnt; //0x08
    unsigned int motDataNum[16]; //0x0C
} EFFECT_DATA;
...
static EFFECT_DATA effectDataTbl[] = {
    {
        EFFECT_ANM_danger,
        10,
        HU3D_PARTICLE_BLEND_NORMAL,
        2,
        { CHARMOT_FILE(c000m1_326), CHARMOT_FILE(c000m1_327) }
    },
    {
        EFFECT_ANM_dust,
        150,
        ...
        0,
        { 
            CHARMOT_FILE(c000m1_301),
            CHARMOT_FILE(c000m1_302),
            CHARMOT_FILE(c000m1_355),
            CHARMOT_FILE(c000m1_326),
            CHARMOT_FILE(c000m1_343),
        }
    },
    {
        EFFECT_ANM_smoke,
    ...
    },
    {
        ...
        2,
        { 
            CHARMOT_FILE(c000m1_322),
            CHARMOT_FILE(c000m1_323),
            CHARMOT_FILE(c000m1_671)
        }
    },
    {
        EFFECT_ANM_cry,
        ...
        0,
        { 
            CHARMOT_FILE(c000m1_354),
            CHARMOT_FILE(c000m1_355)
        }
    },
    {
        EFFECT_ANM_land_dust,
        100,
        HU3D_PARTICLE_BLEND_NORMAL,
        0,
        { 
            CHARMOT_FILE(c000m1_310),
            CHARMOT_FILE(c000m1_304)
        }
    },
};

char *CharHeadObjNameTbl[GW_CHARA_MAX+1] = {
    "root_head",
    "root_head",
    "root_hair1",
    "ske_head",
...
unsigned int CharDataDirTbl[GW_CHARA_MAX][6] = {
    {
        DATA_mariomdl0,
        DATA_mariomdl1,
        DATA_mariomdl2,
...
{
    if(charNo >= GW_CHARA_MAX || charNo < 0 || charNo == (u8)GW_CHARA_NULL) {
        return;
    } else {
        CHAR_WORK *workP = &CharWork[charNo];
        if(!workP->motAMemP) {
...
void CharMotionDefrag(s16 *charTbl)
{
    ...
    CHAR_WORK *workTbl[GW_PLAYER_MAX]; //sp+0x18
    s16 charOrder[GW_PLAYER_MAX]; //sp+0x10
    s16 loadOrder[GW_PLAYER_MAX]; //sp+0x8
    tick = OSGetTick();
    for(work=&CharWork[0], i=charNum=0; i<GW_CHARA_MAX; i++, work++) {
        if(work->motAMemP) {
            charOrder[charNum] = i;
            workTbl[charNum++] = work;
        }
    }
    for(i=charNum-1; i>=0; i--) {
        for(j=0; j<i; j++) {
            if(workTbl[j]->motAMemP > workTbl[j+1]->motAMemP) {
                num = charOrder[j];
                charOrder[j] = charOrder[j+1];
                charOrder[j+1] = num;
                work = workTbl[j];
                workTbl[j] = workTbl[j+1];
                workTbl[j+1] = work;
            }
        }
    }
    for(i=num=0; i<charNum; i++) {
        for(j=0; j<charNum; j++) {
            if(charTbl[j] == charOrder[i]) {
                num++;
                loadOrder[i] = j;
            }
        }
    ...
    for(i=charNum-1; i>=0; i--) {
        for(j=0; j<i; j++) {
            if(loadOrder[j] > loadOrder[j+1]) {
                motDir1 = HuAR_ARAMtoMRAM(workTbl[j]->motAMemP);
                while(HuARDMACheck());
                motDir2 = HuAR_ARAMtoMRAM(workTbl[j+1]->motAMemP);
                while(HuARDMACheck());
                HuARFree(workTbl[j]->motAMemP);
                HuARFree(workTbl[j+1]->motAMemP);
                workTbl[j+1]->motAMemP = HuAR_MRAMtoARAM2(motDir2);
                while(HuARDMACheck());
                workTbl[j]->motAMemP = HuAR_MRAMtoARAM2(motDir1);
                while(HuARDMACheck());
                stat = HuDataGetStatus(motDir1);
                HuDataDirClose(stat->dirId << 16);
                stat = HuDataGetStatus(motDir2);
                HuDataDirClose(stat->dirId << 16);
                num = loadOrder[j];
                loadOrder[j] = loadOrder[j+1];
                loadOrder[j+1] = num;
                num = charOrder[j];
                charOrder[j] = charOrder[j+1];
                charOrder[j+1] = num;
...
static void CharTimingHook(HU3D_MODELID modelId, HU3D_MOTIONID motId, BOOL lagF)
{
    CHAR_WORK *workP = &CharWork[0];
    s16 i;
    for(i=0; i<GW_CHARA_MAX; i++, workP++) {
...
static EFFECT_PARAM dustEffParam = {
    0,
    { 128, 128, 128, 255 },
    { 64, 32, 0, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM hitEffParam = {
    0,
    { 255, 64, 64, 128 },
    { 255, 64, 64, 128 },
    { 0, 0, 0 },
...
static EFFECT_PARAM npcHitEffParam = {
    0,
    { 255, 255, 0, 255 },
    { 255, 255, 0, 255 },
    { 0, 0, 0 },
...
static EFFECT_PARAM warnEffParam = {
    0,
    { 255, 255, 255, 255 },
    { 255, 255, 255, 255 },
    { 0, 20, 0 },
...
static EFFECT_PARAM smokeEffParam = {
    0,
    { 128, 32, 32, 255 },
    { 0, 0, 0, 255 },
    { 0, 10, 0 },
...
static s8 walkVoiceTimeTbl[GW_CHARA_MAX*4] = {
    10, 32, -1, -1,
    20, 40, -1, -1,
    12, 37, -1, -1,
    4, 25, -1, -1,
...
static s8 runVoiceTimeTbl[GW_CHARA_MAX*4] = {
    15, 29, -1, -1,
    5, 19, -1, -1,
    5, 22, -1, -1,
    12, 28, -1, -1,
...
static s8 jumpVoiceTimeTbl[GW_CHARA_MAX*2] = {
    2, 4,
    3, 5,
    2, 3,
    3, -1,
...
static u8 boardWinAnimLen[GW_CHARA_MAX] = {
    14,
    0,
    46,
    72,
...
static u8 winAnimLen[GW_CHARA_MAX] = {
    18,
    0,
    30,
    106,
...
static u8 handUpAnimLen[GW_CHARA_MAX] = {};

static u8 okAnimLen[GW_CHARA_MAX] = {
    0,
    0,
    24,
    90,
...
static u8 winAnimLen2[GW_CHARA_MAX] = {
    18,
    0,
    30,
    106,
...
static u8 holdUpAnimLen[GW_CHARA_MAX] = {
    0,
    6,
    0,
    78,
...
static u8 winJumpUpAnimLen[GW_CHARA_MAX] = {};

static u8 loseAnimLen[GW_CHARA_MAX] = {
    18,
    0,
    36,
    106,
...
static s8 jumpAnimLen[GW_CHARA_MAX] = {
    42,
    42,
    43,
    52,
...
static void UpdateMotPlay(CHAR_WORK *workP, s16 motNo, s16 motNoShift);
...
static void UpdateChar(void)
{
    ...
    CHAR_WORK *workP = &CharWork[*property];
...
static s32 GetMotNoPlayTime(CHAR_WORK *workP, s16 motNo)
{
    s16 i;
    for(i=0; i<2; i++) {
        if(workP->motNoPlay[i] == motNo) {
...
static s16 EffectDustCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param);
static s16 EffectHitCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param);
static s16 EffectSmokeCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param);

static s16 EffectWarnCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, float ofsY, EFFECT_PARAM *param);
static s16 EffectBirdCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, s16 charNo, float ofsY, EFFECT_PARAM *param);
...
static void UpdateCharAnim(s16 charNo, HU3D_MODELID modelId, s16 motNo, u8 voiceFlag, s16 frameNo, HuVecF *ofs)
{
    ...
    switch(motNo) {
        case CHAR_MOTNO(c000m1_315):
        case CHAR_MOTNO(c000m1_318):
             if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(9), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_301):
        case CHAR_MOTNO(c000m1_343):
        case CHAR_MOTNO(c000m1_610):
        case CHAR_MOTNO(c000m1_637):
        case CHAR_MOTNO(c000m1_647):
            ...
            if((frameNo & 0xF) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
            ...
            for(i=0; i<4; i++) {
                if(frameNo == walkVoiceTimeTbl[(charNo*4)+i]) {
                    PlayStepVoice(charNo, MSM_SE_CHAR(0), voiceFlag);
                    break;
                }
            }
            break;
        
        case CHAR_MOTNO(c000m1_302):
        case CHAR_MOTNO(c000m1_355):
        case CHAR_MOTNO(c000m1_611):
        case CHAR_MOTNO(c000m1_638):
        case CHAR_MOTNO(c000m1_651):
            if(!motShiftF && modelP->motWork.speed <= 0.5) {
                break;
            }
            if(motShiftF && modelP->motShiftWork.speed <= 0.5) {
            ...
            if((frameNo & 0x7) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
            ...
            if(motNo == CHAR_MOTNO(c000m1_355) && (frameNo & 0x1)) {
                CharModelCryCreate(charNo, 30, 0.8f);
            }
            for(i=0; i<4; i++) {
                if(frameNo == runVoiceTimeTbl[(charNo*4)+i]) {
                    PlayStepVoice(charNo, MSM_SE_CHAR(4), voiceFlag);
                    break;
                }
            }
            break;
        
        case CHAR_MOTNO(c000m1_304):
            if(frameNo == 5 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                CharModelLandDustCreate(charNo, &modelP->pos);
            }
            for(i=0; i<2; i++) {
                if(frameNo == jumpVoiceTimeTbl[(charNo*2)+i]) {
                    PlayStepVoice(charNo, MSM_SE_CHAR(8), voiceFlag);
                    break;
                }
            }
            break;
        
        case CHAR_MOTNO(c000m1_668):
            if(frameNo == 1 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                CharModelLandDustCreate(charNo, &modelP->pos);
                PlayStepVoice(charNo, MSM_SE_CHAR(8), voiceFlag);
            }
            break;
            
        case CHAR_MOTNO(c000m1_305):
            Hu3DModelObjMtxGet(modelId, CharModelItemHookGet(charNo, workP->model, 0), hitMtx);
            if(frameNo == 0) {
                _CharFXPlay(charNo, MSM_SE_CHAR(22), voiceFlag);
            }
            dotMax = 10;
            if(frameNo <= dotMax && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
        ...
        case CHAR_MOTNO(c000m1_404):
            if(frameNo == 0) {
                _CharFXPlay(charNo, MSM_SE_CHAR(23), voiceFlag);
            }
            if(frameNo < 10) {
        ...
        case CHAR_MOTNO(c000m1_310):
            if(frameNo == 0) {
                if(!(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                    CharEffectHipDropCreate(charNo, &modelP->pos);
                }
                _CharFXPlay(charNo, MSM_SE_CHAR(18), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_326):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(8), voiceFlag);
            }
            if(modelP->motIdShift == HU3D_MOTIONID_NONE || motShiftF) {
                if(!(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                    if(frameNo == 10) {
                        if(charNo == GW_CHARA_WALUIGI) {
                            warnSize = 190;
                        } else if(charNo == GW_CHARA_PEACH || charNo == GW_CHARA_DAISY) {
                            warnSize = 140;
                        } else {
                            warnSize = 120;
                        }
        ...
        case CHAR_MOTNO(c000m1_322):
        case CHAR_MOTNO(c000m1_323):
        case CHAR_MOTNO(c000m1_671):
            if(!(workP->attr & CHAR_ATTR_BIRD) && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHAR(19), voiceFlag);
                for(i=0; i<3; i++) {
                    EffectBirdCreate(modelId, modelP->pos.x, modelP->pos.y+(100.0f*modelP->scale.x), modelP->pos.z, 1.0f, charNo, i*120, &warnEffParam);
                }
                workP->attr |= CHAR_ATTR_BIRD;
            }
            break;
        
        case CHAR_MOTNO(c000m1_321):
            if(!(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                if(frameNo & 0x1) {
                ...
                if(GetMotNoPlayTime(workP, motNo) == 0) {
                    _CharFXPlay(charNo, MSM_SE_CHARVOICE(3), voiceFlag);
                }
            }
            break;
        
        case CHAR_MOTNO(c000m1_303):
            if(frameNo == 0) {
                _CharFXPlay(charNo, MSM_SE_CHAR(16), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_308):
            if(frameNo == 0) {
                _CharFXPlay(charNo, MSM_SE_CHAR(17), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_306):
            if(!(workP->attr & (CHAR_ATTR_VOICEFLAG|CHAR_ATTR_MOT_WIN))) {
                if((omcurovl < DLL_w01dll || omcurovl > DLL_w11dll) && (omcurovl < DLL_s01dll || omcurovl > DLL_s03dll)) {
                    if(frameNo == winAnimLen[charNo]) {
                        ...
                        workP->attr |= CHAR_ATTR_MOT_WIN;
                        attrOld |= CHAR_ATTR_MOT_WIN;
                    }
                }
            }
            break;
        
        case CHAR_MOTNO(c000m1_356):
            if(!(workP->attr & (CHAR_ATTR_VOICEFLAG|CHAR_ATTR_MOT_WIN))) {
                if(frameNo == handUpAnimLen[charNo]) {
                    _CharFXPlay(charNo, MSM_SE_CHARVOICE(6), voiceFlag);
                    workP->attr |= CHAR_ATTR_MOT_WIN;
                    attrOld |= CHAR_ATTR_MOT_WIN;
                }
            }
        ...
        case CHAR_MOTNO(c000m1_385):
            if(!(workP->attr & (CHAR_ATTR_VOICEFLAG|CHAR_ATTR_MOT_WIN))) {
                if(frameNo == winJumpUpAnimLen[charNo]) {
                    _CharFXPlay(charNo, MSM_SE_CHARVOICE(2), voiceFlag);
                    workP->attr |= CHAR_ATTR_MOT_WIN;
                    attrOld |= CHAR_ATTR_MOT_WIN;
                }
            }
        ...
        case CHAR_MOTNO(c000m1_307):
        case CHAR_MOTNO(c000m1_357):
        case CHAR_MOTNO(c000m1_360):
        case CHAR_MOTNO(c000m1_362):
        case CHAR_MOTNO(c000m1_364):
            if(frameNo == 0 && !(workP->attr & (CHAR_ATTR_VOICEFLAG|CHAR_ATTR_MOT_LOSE))) {
                workP->attr |= CHAR_ATTR_MOT_LOSE;
                attrOld |= CHAR_ATTR_MOT_LOSE;
            }
            break;
        
        case CHAR_MOTNO(c000m1_354):
            if(!(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                if(frameNo & 0x1) {
        ...
        case CHAR_MOTNO(c000m1_346):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(11), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_311):
            if(frameNo == jumpAnimLen[charNo] && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                CharModelLandDustCreate(charNo, &modelP->pos);
                for(i=0; i<2; i++) {
                    if(frameNo == jumpVoiceTimeTbl[(charNo*2)+i]) {
                        PlayStepVoice(charNo, MSM_SE_CHAR(8), voiceFlag);
        ...
        case CHAR_MOTNO(c000m1_316):
        case CHAR_MOTNO(c000m1_319):
        case CHAR_MOTNO(c000m1_438):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(9), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_368):
        case CHAR_MOTNO(c000m1_437):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(15), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_376):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(3), voiceFlag);
            }
            break;
        
        case CHAR_MOTNO(c000m1_423):
            if(GetMotNoPlayTime(workP, motNo) == 0 && !(workP->attr & CHAR_ATTR_VOICEFLAG)) {
                _CharFXPlay(charNo, MSM_SE_CHARVOICE(8), voiceFlag);
            }
            break;
    ...
    if(!motShiftF) {
        if(!(attrOld & CHAR_ATTR_MOT_LOSE)) {
            workP->attr &= ~CHAR_ATTR_MOT_LOSE;
        }
        if(!(attrOld & CHAR_ATTR_MOT_WIN)) {
            workP->attr &= ~CHAR_ATTR_MOT_WIN;
        }
    }
}
...
s32 _CharFXPlay(s16 charNo, s16 seNo, u8 voiceFlag)
{
    ...
    if(workP->attr & CHAR_ATTR_PAN_AUTO) {
...
static s16 winVoiceTbl[][2] = {
    DLL_m622dll, MSM_SE_CHARVOICE(6),
    DLL_m623dll, MSM_SE_CHARVOICE(6),
    DLL_m642dll, MSM_SE_CHARVOICE(6),
    DLL_m671dll, MSM_SE_CHARVOICE(6),
    DLL_m672dll, MSM_SE_CHARVOICE(6),
    DLL_m673dll, MSM_SE_CHARVOICE(6),
    DLL_m674dll, MSM_SE_CHARVOICE(6),
    DLL_m675dll, MSM_SE_CHARVOICE(6),
    DLL_m676dll, MSM_SE_CHARVOICE(6),
    DLL_NONE, 0
};
...
static s16 CharWinVoicePlay(s16 charNo, u8 voiceFlag)
{
    ...
    if(winVoiceTbl[i][0] != DLL_NONE) {
        ...
        seNo = MSM_SE_CHARVOICE(4);
    }
    _CharFXPlay(charNo, seNo, voiceFlag);
    return ret;
}

static float CharYOfsTbl1[GW_CHARA_MAX] = {
    110,
    160,
    110,
    160,
...
static float CharYOfsTbl2[GW_CHARA_MAX] = {
    110,
    160,
    110,
    160,
...
static void EffectInit(void)
{
    ...
    for(cameraNo=0; cameraNo<HU3D_CAM_MAX; cameraNo++) {
        ...
        for(i=0; i<EFFECT_MAX; i++) {
            ...
            if(!effParamAll[i]) {
                effParamAll[i] = HuMemDirectMalloc(HEAP_HEAP, effectDataTbl[i].maxCnt*sizeof(EFFECT_PARAM));
            }
...
static void EffectParticleCreate(u8 type)
{
    ...
    for(cameraNo=0; cameraNo<HU3D_CAM_MAX; cameraNo++) {
        ...
        if(effModelId[type][cameraNo] != HU3D_MODELID_NONE) {
            ...
            if(!effParamAll[type]) {
                effParamAll[type] = HuMemDirectMalloc(HEAP_HEAP, effectDataTbl[type].maxCnt*sizeof(EFFECT_PARAM));
            }
...
static s16 EffectCreate(s16 type, s16 cameraBit, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param);

static s16 EffectDustCreate(s16 modelId, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param)
{
    HU3D_MODEL *modelP = &Hu3DData[modelId];
    if(modelP->scale.x != 1.0) {
        EFFECT_PARAM paramNew = *param;
...
static s16 EffectStarCreate(HU3D_MODELID modelId, float posX, float posY, float posZ, float scale, EFFECT_PARAM *param)
{
    HU3D_MODEL *modelP = &Hu3DData[modelId];
    if(modelP->scale.x != 1.0) {
        EFFECT_PARAM paramNew = *param;
...
static void EffectParticleHook(HU3D_MODEL *modelP, HU3D_PARTICLE *particleP, Mtx mtx)
{
    EFFECT_PARAM *param = particleP->work;
    HU3D_PARTICLE_DATA *particleDataP;
    s16 color;
    s16 i;
...
static float birdYOfsTbl[GW_CHARA_MAX] = {
    80,
    80,
    60,
    75,
...
static void UpdateBirdEffect(HU3D_PARTICLE_DATA *particleDataP)
{
    ...
    for(effectNo=0; effectDataTbl[EFFECT_BIRD].motDataNum[effectNo]; effectNo++) {
        if(workP->motNoCurr == FILENUM(effectDataTbl[EFFECT_BIRD].motDataNum[effectNo])) {
            break;
        }
    }
    ...
    particleDataP->pos.y += (birdYOfsTbl[charNo]*modelP->scale.y);
    ...
    if(!birdMotPlayF && particleDataP->time > 30) {
        ...
        if(particleDataP->scale < 0.0f) {
            ...
            if(particleDataP->vel.y == 0.0) {
                _CharFXPlay(charNo, MSM_SE_CHAR(24), workP->attr);
            }
        }
    }
}
...
static s16 stepTbl[] = {
    DLL_w01dll, CHAR_STEP0,
    DLL_w02dll, CHAR_STEP0,
    DLL_w03dll, CHAR_STEP0,
    DLL_w04dll, CHAR_STEP0,
    DLL_w05dll, CHAR_STEP0,
    DLL_w06dll, CHAR_STEP0,
    DLL_w10dll, CHAR_STEP0,
    DLL_w11dll, CHAR_STEP0,
    DLL_s01dll, CHAR_STEP0,
    DLL_s02dll, CHAR_STEP0,
    DLL_s03dll, CHAR_STEP0,
    DLL_mdseldll, CHAR_STEP1,
    DLL_mdminidll, CHAR_STEP1,
    DLL_mdmicdll, CHAR_STEP1,
    DLL_mdpartydll, CHAR_STEP1,
    DLL_mdsingdll, CHAR_STEP1,
    DLL_mdbankdll, CHAR_STEP1,
    DLL_miraclebookdll, CHAR_STEP1,
    DLL_mdpresultdll, CHAR_STEP1,
    DLL_mgmfreedll, CHAR_STEP0,
    DLL_mgmdecathlondll, CHAR_STEP0,
    DLL_mgmbattledll, CHAR_STEP0,
    DLL_mgmtournamentdll, CHAR_STEP0,
    DLL_mgmbingodll, CHAR_STEP2,
    DLL_mgmrenshodll, CHAR_STEP0,
    DLL_micquizdll, CHAR_STEP0,
    DLL_micquizishidll, CHAR_STEP0,
    DLL_micquizmyokodll, CHAR_STEP0,
    DLL_micquizohdedll, CHAR_STEP0,
    DLL_mikeactdll, CHAR_STEP0,
    DLL_m601dll, CHAR_STEP0,
    DLL_m602dll, CHAR_STEP0,
    DLL_m603dll, CHAR_STEP0,
    DLL_m604dll, CHAR_STEP1,
    DLL_m605dll, CHAR_STEP0,
    DLL_m606dll, CHAR_STEP0,
    DLL_m607dll, CHAR_STEP0,
    DLL_m608dll, CHAR_STEP2,
    DLL_m609dll, CHAR_STEP1,
    DLL_m610dll, CHAR_STEP0,
    DLL_m611dll, CHAR_STEP0,
    DLL_m612dll, CHAR_STEP0,
    DLL_m613dll, CHAR_STEP2,
    DLL_m614dll, CHAR_STEP0,
    DLL_m615dll, CHAR_STEP0,
    DLL_m616dll, CHAR_STEP0,
    DLL_m617dll, CHAR_STEP1,
    DLL_m618dll, CHAR_STEP1,
    DLL_m619dll, CHAR_STEP0,
    DLL_m620dll, CHAR_STEP1,
    DLL_m621dll, CHAR_STEP2,
    DLL_m622dll, CHAR_STEP0,
    DLL_m623dll, CHAR_STEP0,
    DLL_m624dll, CHAR_STEP0,
    DLL_m625dll, CHAR_STEP1,
    DLL_m626dll, CHAR_STEP2,
    DLL_m627dll, CHAR_STEP1,
    DLL_m628dll, CHAR_STEP0,
    DLL_m629dll, CHAR_STEP1,
    DLL_m630dll, CHAR_STEP1,
    DLL_m631dll, CHAR_STEP1,
    DLL_m632dll, CHAR_STEP1,
    DLL_m633dll, CHAR_STEP0,
    DLL_m634dll, CHAR_STEP0,
    DLL_m635dll, CHAR_STEP2,
    DLL_m636dll, CHAR_STEP1,
    DLL_m637dll, CHAR_STEP1,
    DLL_m638dll, CHAR_STEP0,
    DLL_m639dll, CHAR_STEP0,
    DLL_m640dll, CHAR_STEP0,
    DLL_m641dll, CHAR_STEP0,
    DLL_m642dll, CHAR_STEP0,
    DLL_m643dll, CHAR_STEP0,
    DLL_m644dll, CHAR_STEP0,
    DLL_m645dll, CHAR_STEP0,
    DLL_m646dll, CHAR_STEP1,
    DLL_m647dll, CHAR_STEP1,
    DLL_m648dll, CHAR_STEP2,
    DLL_m649dll, CHAR_STEP1,
    DLL_m650dll, CHAR_STEP0,
    DLL_m651dll, CHAR_STEP0,
    DLL_m652dll, CHAR_STEP0,
    DLL_m653dll, CHAR_STEP0,
    DLL_m654dll, CHAR_STEP2,
    DLL_m655dll, CHAR_STEP0,
    DLL_m656dll, CHAR_STEP0,
    DLL_m657dll, CHAR_STEP1,
    DLL_m658dll, CHAR_STEP0,
    DLL_m659dll, CHAR_STEP0,
    DLL_m660dll, CHAR_STEP0,
    DLL_m661dll, CHAR_STEP0,
    DLL_m662dll, CHAR_STEP1,
    DLL_m663dll, CHAR_STEP2,
    DLL_m664dll, CHAR_STEP1,
    DLL_m665dll, CHAR_STEP0,
    DLL_m666dll, CHAR_STEP0,
    DLL_m667dll, CHAR_STEP0,
    DLL_m668dll, CHAR_STEP0,
    DLL_m669dll, CHAR_STEP0,
    DLL_m670dll, CHAR_STEP0,
    DLL_m671dll, CHAR_STEP0,
    DLL_m672dll, CHAR_STEP0,
    DLL_m673dll, CHAR_STEP0,
    DLL_m674dll, CHAR_STEP1,
    DLL_m675dll, CHAR_STEP0,
    DLL_m676dll, CHAR_STEP1,
    DLL_m677dll, CHAR_STEP0,
    DLL_m678dll, CHAR_STEP0,
    DLL_m679dll, CHAR_STEP0,
    DLL_m680dll, CHAR_STEP0,
    DLL_m681dll, CHAR_STEP1,
    DLL_m699dll, CHAR_STEP0,
    DLL_NONE, -1
};
...
{
    s16 i;
    for(i=0; stepTbl[i] != DLL_NONE; i++) {
        if(omcurovl == stepTbl[(i*2)+0]) {
            return stepTbl[(i*2)+1];
        }
    }
    return 0;
}
...
static void MotionParticleInit(unsigned int dataNum)
{
    ...
    for(i=0; i<EFFECT_MAX; i++) {
        for(j=0; effectDataTbl[i].motDataNum[j]; j++) {
            if(dataNum == FILENUM(effectDataTbl[i].motDataNum[j])) {
...
char *CharEyeBmpNameTbl[GW_CHARA_MAX*CHAR_MODEL_MAX*2] = {
    "s3c000m1_eyes",
    "s3c000m1_eyes",
    "s3c000m1_eyes",
    "s3c000m1_eyes",
...
static char *CharHookNameTbl[GW_CHARA_MAX*5] = {
    "a-itemhook-r",
    "a-itemhook-l",
    "a-itemhook-fr",
    "a-itemhook-fl",
...
static float modelHeightTbl[GW_CHARA_MAX] = {
    150,
    170,
    188,
    169,
...
static EFFECT_PARAM modelSmokeEffParam = {
    0,
    { 255, 255, 255, 255 },
    { 64, 32, 0, 255 },
    { 0, 2, 1 },
...
void CharEffectSmokeCreateScale(s16 cameraBit, HuVecF *pos, float scale)
{
    ...
    for(bit=HU3D_CAM0, cameraNo=0; cameraNo<HU3D_CAM_MAX; cameraNo++, bit <<= 1) {
        ...
        for(i=0; i<8; i++) {
            ...
            particleDataP->vel.x = 30*HuSin(i*45)*scale;
            particleDataP->vel.y = 30*HuCos(i*45)*scale;
            particleDataP->vel.z = 0;
            particleDataP->accel = *pos;
            particleDataP->speedDecay = 1.15f;
            particleDataP->colorIdx = 0.1f*(frandmod(20)-10)*scale;
        ...
        for(i=0; i<8; i++) {
            ...
            particleDataP = &particleP->data[effectNo];
            particleDataP->parManId = 2;
            particleDataP->vel.x = (frandmod(100)-50)*scale;
            particleDataP->vel.y = (frandmod(100)-50)*scale;
            particleDataP->vel.z = (frandmod(100)-50)*scale;
            particleDataP->accel = *pos;
            particleDataP->speedDecay = 1.15f;
            particleDataP->colorIdx = (0.1f*(frandmod(20)-10))*scale;
            particleDataP->color.a = 255-(frandmod(3)*16);
...
{
    CharEffectSmokeCreateScale(cameraBit, pos, 1);
}
...
static EFFECT_PARAM coinEffParam = {
    1,
    { 255, 255, 0, 255 },
    { 255, 255, 0, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM modelHitEffParam = {
    0,
    { 255, 255, 0, 255 },
    { 255, 128, 0, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM hitGlowEffParam = {
    0,
    { 224, 255, 32, 255 },
    { 224, 255, 32, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM shoeHitEffParam = {
    0,
    { 32, 32, 255, 255 },
    { 128, 255, 32, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM shoeHitGlowEffParam = {
    0,
    { 32, 224, 32, 255 },
    { 32, 224, 32, 255 },
    { 0, 2, 1 },
...
void CharModelVoiceFlagSet(s16 charNo, BOOL fxFlag)
{
    ...
    if(charNo >= GW_CHARA_MAX) {
        ...
        if(!fxFlag) {
            workP->attr |= CHAR_ATTR_VOICEFLAG;
        } else {
            workP->attr &= ~CHAR_ATTR_VOICEFLAG;
        }
    }
...
static u16 npcSeTbl[12] = {
    MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0),
    MSM_SE_CHAR(0), MSM_SE_CHAR(0), 288, 2590,
    286, 273, 59367, 59367
};

static u16 npcSeTbl2[7] = { MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4) };
...
static u16 npcSeTbl3[4] = { MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0) };
...
static u16 npcSeTbl4[4] = { MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4), MSM_SE_CHAR(4) };
...
static u16 npcSeTbl5[4] = {MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0), MSM_SE_CHAR(0) };
...
void CharModelStepSet(s16 charNo, s16 step)
{
    CHAR_WORK *workP = &CharWork[charNo];
    workP->step = step;
}
...
static s16 PlayStepVoice(s16 charNo, s16 seId, u8 voiceFlag)
{
    ...
    if(workP->step == CHAR_STEP_MIKEACT1) {
        seId = MSM_SE_MIKEACT_12|0x8000;
    } else if(workP->step == CHAR_STEP_MIKEACT2) {
        seId = MSM_SE_MIKEACT_20|0x8000;
    } else if(workP->step == CHAR_STEP6) {
        seId = MSM_SE_CHAR(25);
    } else {
        seId += workP->step;
    }
...
static EFFECT_PARAM landEffParam = {
    0,
    { 255, 255, 255, 255 },
    { 255, 255, 255, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM hipDropLandEffParam = {
    0,
    { 255, 255, 255, 255 },
    { 255, 255, 255, 255 },
    { 0, 2, 1 },
...
static EFFECT_PARAM hipDropStarEffParam = {
    0,
    { 255, 255, 0, 255 },
    { 255, 255, 0, 255 },
    { 0, 0, 0 },
...
void CharModelLandDustCreateStep(s16 charNo, HuVecF *pos)
{
    ...
    PlayStepVoice(charNo, MSM_SE_CHAR(8), 0);
}
...
static EFFECT_PARAM cryEffParam = {
    0,
    { 128, 255, 255, 255 },
    { 255, 255, 255, 255 },
    { 0, 2, 1 },
...
void CharModelCryCreate(s16 charNo, float yOfs, float ofsY)
{
    ...
    if(charNo == GW_CHARA_WARIO) {
        pos.y -= 40*modelP->scale.x;
    }
    pos.y += yOfs;
...
static void PlayWinLoseVoice(void)
{
    ...
    CHAR_WORK *workP = &CharWork[winLose->charNo];
    s16 i;
    s16 motNo;
...
{
    if(charNo1 != GW_CHARA_NULL) {
        _CharFXPlay(charNo1, MSM_SE_CHARVOICE(12), 0);
    }
    if(charNo2 != GW_CHARA_NULL) {
        _CharFXPlay(charNo2, MSM_SE_CHARVOICE(12), 0);
    }
    if(charNo3 != GW_CHARA_NULL) {
        _CharFXPlay(charNo3, MSM_SE_CHARVOICE(12), 0);
    }
    if(charNo4 != GW_CHARA_NULL) {
        _CharFXPlay(charNo4, MSM_SE_CHARVOICE(12), 0);
    }
}
...
{
    CHAR_WORK *work = &CharWork[0];
    s16 i;
    s16 j;
    HU3D_LLIGHTID LLightID;
    HU3D_MODEL *modelP;
    LLightID = HU3D_LIGHTID_NONE;
    for(i=0; i<GW_CHARA_MAX; i++, work++) {
        if(work->modelId == HU3D_MODELID_NONE) {
            continue;
        }
        if(LLightID == HU3D_LIGHTID_NONE) {
            LLightID = Hu3DLLightCreateV(work->modelId, pos, dir, color);
            if(LLightID == HU3D_LIGHTID_NONE) {
...
static inline HU3D_LIGHT *CharLightGet(void)
{
    CHAR_WORK *work = &CharWork[0];
    s16 i;
    s16 j;
...
{
    HU3D_LIGHT *lightP = CharLightGet();
    if(lightP) {
...
{
    CHAR_WORK *work = &CharWork[charNo];
    HU3D_MODEL *modelP = &Hu3DData[work->modelId];
    EffectWarnCreate(work->modelId, modelP->pos.x, modelP->pos.y+100.0f, modelP->pos.z, 20, ofsY, &warnEffParam);
}
