...
| `board/coin.c` | Forty-four allocation, lifecycle, transform, alpha, display, layer, motion, renderer, and effect routines are recovered. The target proves the `0x40` public object, its caller-work tail, `0x1144` model/effect bank, real `0x160` `MBCOINOBJDATA`, and `0x1C` display-model entries. Target/source `.text` are `0x4550/0x32C0` at 63.219566% data-value; 44/52 functions pair and 16 are exact. The ten-function display family represents `0x156C` target bytes and seven functions are exact; three helper-expansion residues keep the owner fallback-linked. |
...
| `board/star.c` | The target-addressed `STARWORK`, exact `0x60` `STARDISPWORK`, 999-slot star/Ztar banks, four-player display bank, process/animation/hook roots, add/display flow, lifecycle, transform, event/destructor, grow/shrink/effect, and display helpers are recovered. Coverage reaches 86/90 paired and 64 data-value-exact functions. Target/source `.text` are `0x5C00/0x43B0` at 69.696500%; source `.bss 0x1F48` is target-exact and source `.sbss 0x34` covers the target's named prefix. Four target functions remain absent and represented display/event residues keep the owner fallback-linked. |
...
| `board/wipe.c` | All 33 target functions are represented. The lifecycle, dispatcher, material hook, update paths, paper geometry setup, and grid/paper/image GX draw paths recover the semantic `0x2C` `WIPE_SPECIAL_DATA`, `0x14AC` grid, `0xD34` paper, `0x18` image work, real `WIPE_PAPER_VTX`, eight-entry fade table, paper geometry/light globals, and exact target/source `.data 0xA0` and `.bss 0x38`. Six special-wipe lifecycle closures raise coverage to 31/33 data-value-exact functions; target/source `.text` are `0x3664/0x3650` at 99.679690%. Only `mbWipeSpecialInit` and `mbWipeSpecialFadeOutCreate` remain divergent, so the source-complete owner remains fallback-linked. |
...
| `board/config.c` | Matching MP5 pause-copy shape plus MP6 instructions recover the framebuffer/model/counter globals, four-player display state, framebuffer create/kill/draw path, pad-disable setter, pause-guide kill state, and story-mode helper. All six represented functions are data-value exact; target/source `.text` are `0x63BC/0x560` at 5.389316%. The pause panel/guide/config state machines remain absent. |
| `board/last5.c` | The target-derived stable rank/order routine and 40-coin effect recover `0x658` retained target text plus the real `LAST5COINWORK { s16 delay; float velocity; }` overlay on public `MBCOINOBJ` caller work. Target/source `.text` are `0x2458/0x650` at 17.356405%. `Last5PlayerOrderGet` is exact after restoring the authenticated team helper; `ev_Last5Coin40` rises to 98.781910% after recovering the target's floating-point delay calculation. Its one write-only local and the roulette work remain unauthenticated and are not invented. |
...
| `board/shopevent.c` | Six target-backed enable/hook/init/back-create/shop-call routines recover the typed object hook and real two-word `MBSHOPWORK { playerNo, shopNo }`. All six represented functions are data-value exact; target/source `.text` are `0x2FDC/0x16C` at 2.970944%. Target symbol order restores the non-inlined init wrappers, and the shop-call wrapper retains the proven untyped-to-typed allocation lifetime. Seven shop construction/selection functions remain absent. |
...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK` carries typed movement timers, collision state, and Metal/Biri-Q owners while exact `.bss 0x140` retains the four-player bank. Coverage reaches 160/165 paired and 133 data-value-exact functions. Matching MP5 authenticates the shared families; MP6 proves the differing flags, constants, particle `vertexNo`, by-value color ABI, and bin ownership. Target/source `.text` are `0xBEB0/0x9F98` at 81.179940%. `mbPlayerMetalSet` is exact; five target functions remain absent and paired-single effect-builder residues keep the owner fallback-linked. |
