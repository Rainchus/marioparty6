...
| `board/opening.c` | All 22 target functions now recover the party and single opening flows, coin/drop/order helpers, the target `GW_PLAYER::playerNo` bitfield, typed process/hook/effect state, and all initialized owner tables. The new single closure includes its cleanup owner, 256-entry route, corner reduction, adaptive Simpson segment lengths, Newton constant-distance traversal, de Casteljau guide flight, and particle/window/motion/fall sequence. Target/source `.text` are `0x2B14/0x2CE4`; 15/22 functions are exact. `mbev_OpeningSingle` is target/source `0x27C/0x280` at 99.371070%; `ev_OpeningSingle` is `0x1094/0x122C` at 71.556076% because the callback evaluator still inlines instead of retaining the target's indirect call. The complete owner remains fallback-linked. |
...
| `dolphin/mic/m2s.c` | Both calibration-history transfers and the start-time clear now use the target `0x58` bytes (`44 * sizeof(s16)`) rather than the full `0xB0` backing arrays. Separating the initialization/open check from the negative-sample check reproduces the target early control-flow branches in `M2SAdvanceBuffer`. Target/source `.text` remain `0xD50/0xD0C`; the data-value score improves from 78.733567% to 78.991780%, with 1/14 exact and 143/149 relocations. A debug-index lifetime probe grew and regressed the function and was reverted. |
