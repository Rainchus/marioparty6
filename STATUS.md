...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK`, four-entry `playerWork` bank, hooks, object/model/team helpers, transform and motion wrappers, coin/star/capsule state, voices, display, and corner accessors are recovered. Coverage rises from 11 mapped/10 exact to 86/66 strict-exact. Target/source `.text` are `0xBEB0/0x33E8` at 26.040234%, and `.bss 0x140` is target-exact. Twenty represented functions remain 78.666664%-99.545456%, while the large movement/render/state-machine closure remains absent. `branch.c` and `opening.c` now consume the shared owner fields instead of private overlays. |
