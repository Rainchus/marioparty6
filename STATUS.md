...
| `board/tutorial.c` | The real `0x14` call work, three typed ID registries (`s16[16]`, `s16[16]`, and `s16[32]`), `0x80` window-timing bank, guide/process globals, and exit flags are recovered. Target/source `.text` are `0x2C34/0x139C`; 40 functions pair and 34 are exact, up from 10/10. The six residuals are the view setter and three ID-kill/set paths plus their inlined sprite-off form, at 89.250000%-97.073170%. The watch/create, message-table, and minigame-call closures remain absent. |
...
| `board/opening.c` | Twenty of 22 target functions now recover the complete party-opening flow, coin/drop/order helpers, single-event destructor and particle hook, the target `GW_PLAYER::playerNo` bitfield, typed process/hook/effect state, and all initialized owner tables. Target/source `.text` are `0x2B14/0x1838`; 15/20 functions are exact, up from 12/14. The five real large bodies are 95.736336%-97.197365%; `.rodata 0x38`, `.data 0x1C0`, and `.sbss 0x38` are exact, while source BSS `0x994` is followed by the target's four-byte split tail. `mbev_OpeningSingle` and `ev_OpeningSingle` remain omitted because the latter's adaptive curve/Newton camera traversal is not yet authenticated. |
...
| `dolphin/mic/m2s.c` | Both calibration-history transfers now use the target `0x58` bytes (`44 * sizeof(s16)`) rather than copying the full `0xB0` backing arrays. Target/source `.text` remain `0xD50/0xD0C`; the current data-value objdiff score is 78.733570%, with 1/14 exact and 143/149 relocations. A debug-index lifetime probe grew and regressed the function and was reverted. |
