...
| `dolphin/mic/m2s.c` | The target BSS order is recovered as `__M2SBlock` at `0x0`, `__M2STmpHistory` at `0x1A0`, and `__M2SDebug` at `0x250`; source `.bss 0x25C` differs from target `0x260` only by its named four-byte tail. Compound narrow shifts and the cached source/destination/index countdown shape improve `M2SAdvanceBuffer` from 64.012660% (`0x148`) to 74.050640% (`0x140`) against target `0x13C`. Target/source whole `.text` are `0xD50/0xD04`; data-value score is 79.922540%, with 1/14 strict-exact and 143/149 target/source relocations. A `defer_codegen` probe moved calibration and regressed BSS order, so it was removed. |
