...
| `board/coin.c` | Thirty-four allocation, lifecycle, transform, alpha, display, layer, motion, renderer, and effect routines are recovered. The target proves the `0x40` public object, its caller-work tail used by `board/last5.c`, the `0x1144` model/effect bank, the real `0x160` `MBCOINOBJDATA`, and the initialized display/effect tables. Target/source `.text` are `0x4550/0x2204`, at 41.045536% data-value; 34/52 functions pair and 9 are exact. Seven lifecycle/render/effect additions represent `0xF84` target bytes; their remaining instruction differences keep the owner fallback-linked. |
...
| `board/star.c` | The target-addressed `STARWORK`, 999-slot star/Ztar object banks, four-player display bank, process/animation/hook roots, lifecycle, transform, event/destructor, grow/shrink/effect, and display helpers are recovered. Five Star/Ztar flow additions raise coverage to 77/90 paired and 62 data-value-exact functions. Target/source `.text` are `0x5C00/0x2C14` at 45.017155%; source `.bss 0x1F48` is target-exact and source `.sbss 0x34` exactly covers the target's named prefix of `.sbss 0x38`. `ZtarObjRotate` is exact; thirteen target functions remain absent and represented flow residues keep the owner fallback-linked. |
...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK` carries typed movement timers, `posFixObj`, and `HuVecF` collision state while exact `.bss 0x140` retains the four-player bank. Normal/jump/climb movement, rotation, collision, swap, ordering, position reset, and Biri-Q setup recovery raise coverage to 153/165 paired and 132 data-value-exact functions. Matching Mario Party 5 `src/game/board/player.c` at `e246f9d9` authenticates the shared source families; MP6 bytes establish the differing flags, constants, branches, and aggregate copies. Target/source `.text` are `0xBEB0/0x8578` at 68.561040%. Exact `mbPlayerBiriQSet` proves the recovered `GW_PLAYER` bits; twelve target functions remain unpaired and the owner remains fallback-linked. |
