...
| `board/coin.c` | Twenty-four allocation, lifecycle, transform, alpha, display, layer, motion, and display-teardown routines are recovered. The target proves the `0x40` public object, its caller-work tail used by `board/last5.c`, the `0x1144` 64-slot bank, the `0x160` model/bank owner, and the initialized five-entry `coinDispOMObj` bank. Target/source `.text` are `0x4550/0x1038` at 17.027052%; 5/24 mapped functions are strict-exact. New `mbCoinDispKill` is 71.807690% and exact-size `mbCoinDispKillCheck` is 99.800000%; all prior scores are unchanged. The renderer/effect/display closure remains absent. |
...
| `board/star.c` | The target-addressed `STARWORK`, 999-slot star/Ztar object banks, four-player display bank, process/animation/hook roots, lifecycle, transform, event/destructor, grow/shrink/effect, and display helpers are recovered. Coverage rises from 17/17 to 67 mapped/55 strict-exact functions. Target/source `.text` are `0x5C00/0x1564` at 21.559782%; source `.bss 0x1F48` is target-exact and source `.sbss 0x34` exactly covers the target's named prefix of `.sbss 0x38`. Twelve represented functions still diverge and the object creation/update/render closure remains absent. |
...
| `board/wipe.c` | The lifecycle, dispatcher, material hook, and shape/grid/paper/image update paths recover the semantic `0x2C` `WIPE_SPECIAL_DATA`, `0x14AC` grid, `0xD34` paper, `0x18` image work, eight-entry fade table, and exact target/source `.bss 0x38`. All six Wave 59 additions have exact target/source sizes and are data-value exact; `WipeSpecialDraw` is also strict-exact, while the other five retain only pooled-constant symbol ownership differences. Coverage reaches 29 mapped/16 strict-exact and 21 data-value-exact functions. Target/source `.text` are `0x3664/0x1B34`, at 43.738870% strict and 43.845158% data-value. `WipePaperUpdate` and the three remaining draw bodies are absent, so the owner stays fallback-linked. |
...
| `board/opening.c` | All 22 target functions recover the party and single opening flows, coin/drop/order helpers, the target `GW_PLAYER::playerNo` bitfield, shared target-proven `MBPLAYERWORK`, typed process/hook/effect state, and all initialized owner tables. The single closure includes its cleanup owner, 256-entry route, corner reduction, adaptive Simpson segment lengths, Newton constant-distance traversal, de Casteljau guide flight, and particle/window/motion/fall sequence. Target/source `.text` are `0x2B14/0x2CDC` at 87.347840% under the data-value comparison; 15/22 functions are exact there. `mbev_OpeningSingle` is target/source `0x27C/0x280` at 99.371070%; `ev_OpeningSingle` is `0x1094/0x122C` at 71.556076% because the callback evaluator still inlines instead of retaining the target's indirect call. The complete owner remains fallback-linked. |
| `board/scroll.c` | Fifteen scroll/map lifecycle, event, collision cleanup, star-space, animation, bounded player-collision, map-sprite, camera, and hook routines are recovered, 11 strict-exact, up from 9/7. Target/source `.text` are `0x3E4C/0x608` at 9.658892%; `.sbss` is exact at `0x28`. `mbev_Scroll`, `ScrollMain`, `MapViewCreate`, and `mbMapCameraSet` are 99.493670%-99.807690%; the latter two differ only by local data-label identity. Collision construction, map rendering, and their execution closures remain absent. |
...
| `board/capevent.c` | The 28 existing exact event helpers plus ten recovered angle-wrap, Hermite, and quadratic-Bezier helpers reach 38 mapped functions. All 38 are data-value exact; 28 are strict-exact, while the new helpers retain only 100 same-valued local-literal relocation-ownership rows. Target instructions prove the scalar Bezier ABI is `(t, a, b, c)`. Target/source `.text` are `0x14EFC/0x11E0`, at 5.312748% strict and 5.336070% data-value. The other 199 target functions remain absent. |
...
| `dolphin/mic/mic.c` | MP6 `divw`, `srawi`, and `cmpw` instructions prove signed `hw_buff_size`, `buff_size`, `buff_ring_size`, and `buff_ring_cur` fields plus arithmetic `>> 1` sample-index expressions. Target/source `.text` remain `0x2020/0x1F40`; the current data-value objdiff score is 54.690662%, with 0/41 exact and 299/291 relocations. Five functions improve; `__MICTxHandler` now has the target `cmpw` even though its alignment score changes non-monotonically. A signed-timeout and `/ 2` probe were measured and reverted. |
| `dolphin/mic/m2s.c` | The target BSS order is recovered as `__M2SBlock` at `0x0`, `__M2STmpHistory` at `0x1A0`, and `__M2SDebug` at `0x250`; source `.bss 0x25C` differs from target `0x260` only by its named four-byte tail. Direct indexed control-block access keeps `M2SSetBuffer` and `M2SSetMode` byte-exact. Reusing `samples` for the `MICGetSamples` result moves the result lifetime to target register `r30` and improves `M2SGetSamples` from 88.700000% to 88.842860%. Target/source `.text` remain `0xD50/0xD04`; whole scores rise to 79.992960% strict and 80.004690% data-value, strict-exact coverage remains 3/14, and relocations remain 143/149. A regressing walking-pointer close probe was rejected; the complete owner remains fallback-linked. |
...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK` now names `masuMoveF` at `0x0A` and `moveProc` at `0x48`, while exact `.bss 0x140` retains the four-player bank. Recovered turn/movement, movement-number, collision, metal/Biri-Q, and coin-space paths raise coverage from 92 mapped/68 strict-exact to 123/97; 103 functions are data-value exact. The new packed work bitfields and default metal-color bytes come directly from target accesses/data. Target/source `.text` are `0xBEB0/0x4310`, at 33.953457% strict and 33.997707% data-value. `mbPlayerMoveExec` is data-value exact and `mbPlayerEffectSet` has one local branch-destination identity mismatch. Forty-two target functions, including the large movement/render/state-machine closure, remain unpaired. |
