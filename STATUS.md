...
| `board/wipe.c` | All 33 target functions are represented. The lifecycle, dispatcher, material hook, update paths, paper geometry setup, and grid/paper/image GX draw paths recover the semantic `0x2C` `WIPE_SPECIAL_DATA`, `0x14AC` grid, `0xD34` paper, `0x18` image work, real `WIPE_PAPER_VTX`, eight-entry fade table, paper geometry/light globals, and exact target/source `.data 0xA0` and `.bss 0x38`. `WipePaperUpdate`, `WipeGridDraw`, `WipePaperDraw`, and `WipeImageDraw` are data-value exact. Coverage is 33 mapped/16 strict-exact/25 data-value-exact functions; target/source `.text` are `0x3664/0x3744`, at 95.119220% strict and 95.441540% data-value. Eight represented functions still diverge after instruction-value comparison, so the source-complete owner remains fallback-linked. |
...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK` now carries typed movement timers, `posFixObj`, and `HuVecF` collision state while preserving unauthenticated names as unknowns; exact `.bss 0x140` retains the four-player bank. Normal/jump/climb movement, rotation, walk-number display, collision, swap, ordering, and position-reset recovery raise coverage to 150 mapped/117 strict-exact/130 data-value-exact functions. Matching Mario Party 5 `src/game/board/player.c` at `e246f9d9` authenticates the shared source families; MP6 bytes establish the differing flags, constants, branches, and aggregate copies. Target/source `.text` are `0xBEB0/0x80CC`, at 66.051620% strict and 66.114300% data-value. Fifteen target functions remain unpaired: initialization/legacy move-turn paths, metal/Biri-Q creation and updates, and `MasuCoinExec`. The owner remains fallback-linked. |
