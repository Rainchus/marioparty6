...
| `board/config.c` | Matching MP5 pause-copy shape plus MP6 instructions recover the framebuffer/model/counter globals, four-player display state, framebuffer create/kill/draw path, pad-disable setter, pause-guide kill state, story-mode helper, complete target-proven `0x5C` `PAUSE_PANEL_WORK`, panel bank, and guide-kill owner globals. All 14 represented functions are data-value exact, including eight position/rotation/scale/bank/batsu/freeze panel accessors; target/source `.text` are `0x63BC/0x784` at 7.535642% data-value. The pause panel/guide/config state machines remain absent. |
