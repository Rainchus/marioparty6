...
Every configured `src/dolphin/` owner except `dolphin/mic/mic.c` and
`dolphin/mic/m2s.c` is now Matching. Those two MIC owners remain real C
...
| `board/wipe.c` | The create/wait functions, twelve fade/white/dissolve wrappers, special status tail, and six special-wipe lifecycle/state functions are recovered. The lifecycle proves the full semantic `0x2C` `WIPE_SPECIAL_DATA`, three-entry animation bank, two target-backed resource tables, and exact target/source `.bss 0x38`; source now owns `.data 0x18`. Coverage rises to 23 mapped/15 strict-exact functions with no prior regression. Target/source `.text` are `0x3664/0xDA8` at 18.934214% strict and 18.938524% data-value. The special renderer and shape/grid/paper/image update/draw closure remains absent. |
...
| `dolphin/mic/m2s.c` | The target BSS order is recovered as `__M2SBlock` at `0x0`, `__M2STmpHistory` at `0x1A0`, and `__M2SDebug` at `0x250`; source `.bss 0x25C` differs from target `0x260` only by its named four-byte tail. Compound narrow shifts and the cached countdown improve `M2SAdvanceBuffer` to 74.050640% (`0x140`) against target `0x13C`. Direct indexed control-block access makes `M2SSetBuffer` and `M2SSetMode` byte-exact at `0x58`, raising strict-exact coverage to 3/14. Target/source whole `.text` remain `0xD50/0xD04`; scores are 79.981224% strict and 79.992960% data-value, with 143/149 target/source relocations. The complete owner remains fallback-linked. |
...
| `board/player.c` | The target-proven `0x50` `MBPLAYERWORK`, exact four-entry `.bss 0x140` bank, hooks, object/model/team helpers, transform/motion wrappers, coin/star/capsule state, voices, display, corners, close lifecycle, rotation completion, material clone, and character-name owners are recovered. Coverage rises from 86 mapped/66 strict-exact to 92/68; both rotation checks are exact and all prior scores are unchanged. Target/source `.text` are `0xBEB0/0x3704` at 27.636677% strict and 27.680515% data-value. The name owners materialize source `.rodata 0xA4`, `.data 0x14`, and `.sdata 0x3B`. The large movement/render/state-machine closure remains absent; `branch.c` and `opening.c` consume the shared owner fields instead of private overlays. |
